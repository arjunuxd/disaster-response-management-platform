...
const RiskZoneEditModal = ({ isOpen, onClose, zone, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setError(null);
    try {
      await mapService.updateRiskZone(zone._id, {
        ...formData,
      ...
      setError(err.response?.data?.message || 'Failed to update risk zone.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <div className="fixed inset-0 z-[2000] flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-gray-200">
          <div>
            <h2 className="text-lg font-bold text-gray-900">Edit Risk Zone</h2>
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            ...
            >
              {submitting ? 'Updating...' : 'Update Risk Zone'}
            </button>
          </div>
        </form>
      </div>
...
export default RiskZoneEditModal;
